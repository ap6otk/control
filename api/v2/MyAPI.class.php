...
class MyAPI extends API
{
	...
	public function iskrasys () {
		$this->authorise();
		return $this->commonRequest('/','iskrasys.pak');
	}
	...
	public function wipepassive($args) {
		$this->authorise();
		if (!$this->user->isAdmin()) throw new Exception ('Admin user rights required', 403);

		$cur=$this->getCurrent(FALSE, TRUE);
		$fullFileName=$this->userParkDir.'/'.$cur.$filepath.$filename;
		
		$res = removePath($fullFileName);
